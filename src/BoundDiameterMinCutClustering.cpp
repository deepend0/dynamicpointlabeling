...
vector<Graph*>* BoundDiameterMinCutClustering::findComponentsBFS(Graph* graph, int vertexSizeLimit) {
	...
	while((vertex = nextVertex(verticesToBeVisited, vertexNumber))>=0) {
		...
		for(int i=0; i<componentVertexIndices->size() && compareWithInfinity(currentVertexIx, vertexSizeLimit)>0; i++) {
			int vertexToIterate = (*componentVertexIndices)[i];
			vector<int>* adjacencyList = graph->getAdjacencyList() + vertexToIterate;
			for(vector<int>::iterator it2=adjacencyList->begin(); it2!=adjacencyList->end() && compareWithInfinity(currentVertexIx, vertexSizeLimit)>0; it2++) {
				...
				if(adjacentVertexIsFound) {
					...
					componentVertices->push_back(graph->getVertices()->at(adjacentVertex));
		...
		vector<int>* componentAdjacencyList = new vector<int>[componentVertexSize];
		...
		for(int i=0; i<componentVertexSize; i++) {
			...
			for(vector<int>::iterator it2=adjacencyList->begin(); it2!=adjacencyList->end(); it2++) {
				int adjacentVertex = *it2;
				bool adjacentVertexIsFound = false;
				...
				if(adjacentVertexIsFound) {
					componentAdjacencyList[i].push_back(vertexIndicesMap[adjacentVertex]);
				}
			}
		}
...
void BoundDiameterMinCutClustering::floydWarshallAllPairsShortestPaths(Graph* graph, int** shortestPaths) {
	int vertexNumber = graph->getVertexNumber();
	...
	for(int i=0; i<vertexNumber; i++) {
		for(int j=0; j<vertexNumber; j++) {
			for(int k=0; k<vertexNumber; k++) {
				int sumTwoOther = sumWithInfinity(shortestPaths[j][i], shortestPaths[i][k]);
				if(compareWithInfinity(shortestPaths[j][k],sumTwoOther)==-1) {
...
vector<pair<int,int>*>* BoundDiameterMinCutClustering::randomCut(Graph* component, Graph* subComponent1, Graph* subComponent2) {
	...
	vector<pair<int,int>*>** multiGraphAdjMatrixWithSourceVertex = new vector<pair<int,int>*>*[componentSize];
	for(int i=0; i<componentSize; i++) {
		multiGraphAdjMatrixWithSourceVertex[i] = new vector<pair<int,int>*>[componentSize];
	}
	...
	for(int vertex = 0; vertex<componentSize; vertex++) {
		pair<int,vector<int>*>* adjacencyIndicatorListOfVertexPair = new pair<int,vector<int>*>();
		adjacencyIndicatorList.push_back(adjacencyIndicatorListOfVertexPair);
		vector<int>* adjacencyIndicatorListOfVertex = &adjacencyIndicatorMap[vertex];
		adjacencyIndicatorListOfVertexPair->first = vertex;
		adjacencyIndicatorListOfVertexPair->second = adjacencyIndicatorListOfVertex;
		for(vector<int>::iterator it2=component->getAdjacencyList()[vertex].begin(); it2!=component->getAdjacencyList()[vertex].end(); it2++) {
			int adjacentVertex = *it2;
			adjacencyIndicatorListOfVertex->push_back(adjacentVertex);
			multiGraphAdjMatrixWithSourceVertex[vertex][adjacentVertex].push_back(new pair<int,int>(vertex, adjacentVertex));
		}
	}
	...
	while((currentVertexSize = adjacencyIndicatorList.size())>2) {
		...
		pair<int,vector<int>*>* adjacencyIndicatorListOfVertexPair = adjacencyIndicatorList[contractVertexIx1];
		int contractVertex = adjacencyIndicatorListOfVertexPair->first;
		vector<int>* adjacencyIndicatorListOfVertex = adjacencyIndicatorListOfVertexPair->second;
		int targetVertexIx1 = GARandomInt(0,adjacencyIndicatorListOfVertex->size()-1);
		...
		for(int i=0;i<mergedVertices[contractVertex]->size();i++) {
			mergedVertices[targetVertex]->push_back(mergedVertices[contractVertex]->at(i));
			mergedVertexNos[targetVertex]->push_back(mergedVertexNos[contractVertex]->at(i));
		}
		for(int i=0;i<mergedEdges[contractVertex]->size();i++) {
			mergedEdges[targetVertex]->push_back(mergedEdges[contractVertex]->at(i));
		}
		delete mergedVertices[contractVertex];
		delete mergedVertexNos[contractVertex];
		delete mergedEdges[contractVertex];

		for(vector<int>::iterator it=adjacencyIndicatorListOfVertex->begin(); it!=adjacencyIndicatorListOfVertex->end(); it++) {
			int adjacentVertex = *it;
			//cout<<"randomCut>>adjacent vertex: "<<adjacentVertex<<endl;

			vector<int>* adjacencyOfAdjacentVertex = &adjacencyIndicatorMap[adjacentVertex];
			vector<pair<int,int>*>* edgesContractAdjacent = &multiGraphAdjMatrixWithSourceVertex[contractVertex][adjacentVertex];
			vector<pair<int,int>*>* edgesAdjacentContract = &multiGraphAdjMatrixWithSourceVertex[adjacentVertex][contractVertex];
			if(adjacentVertex != targetVertex) {
				vector<pair<int,int>*>* edgesTargetAdjacent = &multiGraphAdjMatrixWithSourceVertex[targetVertex][adjacentVertex];
				vector<pair<int,int>*>* edgesAdjacentTarget = &multiGraphAdjMatrixWithSourceVertex[adjacentVertex][targetVertex];
				...
				if(edgesTargetAdjacent->empty()) {
					...
					vector<int>* adjacencyOfTargetVertex = &adjacencyIndicatorMap[targetVertex];
					adjacencyOfTargetVertex->push_back(adjacentVertex);
				}
				for(int i=0; i<edgesContractAdjacent->size(); i++) {
					edgesTargetAdjacent->push_back(edgesContractAdjacent->at(i));
					edgesAdjacentTarget->push_back(edgesAdjacentContract->at(i));
				}

			} else {
				//cout<<"randomCut>>merge edges"<<endl;
				for(int i=0; i<edgesContractAdjacent->size(); i++) {
					mergedEdges[targetVertex]->push_back(edgesContractAdjacent->at(i));
			...
			for(vector<int>::iterator it3=adjacencyOfAdjacentVertex->begin(); it3!=adjacencyOfAdjacentVertex->end(); it3++) {
		...
		adjacencyIndicatorList.erase(adjacencyIndicatorList.begin()+contractVertexIx1);
	...
	for(vector<int>::iterator it=verticesOfSubComponent->begin(); it!=verticesOfSubComponent->end(); it++) {
		//cout<<*it<<",";
	}
	//cout<<endl;
	buildGraph(vertexNosOfSubComponent, verticesOfSubComponent, edgesOfSubComponent, subComponent1);
	...
	//cout<<endl;
	buildGraph(vertexNosOfSubComponent, verticesOfSubComponent, edgesOfSubComponent, subComponent2);

	vector<pair<int,int>*>* cut = new vector<pair<int,int>*>(multiGraphAdjMatrixWithSourceVertex[lastVertex1][lastVertex2]);
	vector<pair<int,int>*>* copyOfCut = &multiGraphAdjMatrixWithSourceVertex[lastVertex2][lastVertex1];
...
Graph** BoundDiameterMinCutClustering::boundedDiameterMinCut(Graph* component, int diameter) {
	int componentVertexNumber = component->getVertexNumber();
	int componentEdgeNumber = component->getEdgeNumber();
	double avgIncidenceFactor = 2*(double)componentEdgeNumber/(double)componentVertexNumber;
	//cout<<"boundedDiameterMinCut>>diameter limit: "<<diameter<<" vertex number: "<<componentVertexNumber<<" avg inc: "<<avgIncidenceFactor<<endl;
	int numIterations = (int)ceil((componentVertexNumber-1.0)*componentVertexNumber/2.0*log(avgIncidenceFactor));
	...
	for(int i=0; i<numIterations; i++){
		...
		vector<pair<int,int>*>* cut = randomCut(component, subComponent1, subComponent2);
		int cutSize = 0;
		if(cut!=NULL) {
			//cout<<"boundedDiameterMinCut>>cut: ";
			cutSize = cut->size();
			for(vector<pair<int,int>*>::iterator it=cut->begin(); it!=cut->end(); it++) {
		...
		double subComponent1IncidenceFactor = 2.0 * (double)subComponent1->getEdgeNumber() / (double)subComponent1->getVertexNumber();
		double subComponent2IncidenceFactor = 2.0 * (double)subComponent2->getEdgeNumber() / (double)subComponent2->getVertexNumber();
		...
		double expectedCutSize1 = ceil(log2((double)subComponent1Diameter/(double)diameter))*subComponent1IncidenceFactor;
		double expectedCutSize2 = ceil(log2((double)subComponent2Diameter/(double)diameter))*subComponent2IncidenceFactor;
...
vector<Graph*>* BoundDiameterMinCutClustering::cluster(graph::Graph* graph, int diameterLimit) {
	...
	for(int i=0; i<components->size(); i++) {
		...
		if(diameter<=diameterLimit) {
			...
			Graph** subComponents = boundedDiameterMinCut(component, diameterLimit);
...
void BoundDiameterMinCutClustering::buildGraph(vector<int>* vertexNos, vector<int>* vertices, vector<pair<int,int>*>* edges, Graph *graph) {
