...
public:
	...
	void floydWarshallAllPairsShortestPaths(Graph* component, int **shortestPaths);
	int getDiameterOfComponent(Graph* component);
	std::vector<std::pair<int,int>*>* randomCut(Graph* component, Graph* subComponent1, Graph* subComponent2);
...
private:
	...
	void buildGraph(std::vector<int>* vertexNos, std::vector<int>* vertices, std::vector<std::pair<int,int>*>* edges, Graph *graph);
};
